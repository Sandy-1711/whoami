...
// to both stdout and the .log file. The page-count check in check-resume.ts
...
export async function checkLog(
  logPath: string,
  { maxOverfullPt = 2 }: { maxOverfullPt?: number } = {},
): Promise<string[]> {
  let raw: string;
  try {
    ...
    return [`Could not read LaTeX log at ${logPath}: ${(err as Error).message}`];
  }
